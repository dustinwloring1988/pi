...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@buckeyestudio/container-coder")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @buckeyestudio/container-coder",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@buckeyestudio/container-coder")).toBeUndefined();
		expect(getUpdateInstruction("@buckeyestudio/container-coder")).toBe(
			"Update @buckeyestudio/container-coder using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@buckeyestudio/container-coder");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@buckeyestudio/container-coder",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @buckeyestudio/container-coder`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@buckeyestudio/container-coder", undefined, {
			packageName: "@buckeyestudio/container-coder",
			installSpec: "@buckeyestudio/container-coder@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@buckeyestudio/container-coder@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @buckeyestudio/container-coder@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@buckeyestudio/container-coder", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @buckeyestudio/container-coder`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@buckeyestudio/container-coder")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @buckeyestudio/container-coder",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@buckeyestudio/container-coder"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @buckeyestudio/container-coder",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@buckeyestudio/container-coder";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@buckeyestudio/container-coder")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@buckeyestudio/container-coder")).toContain(
