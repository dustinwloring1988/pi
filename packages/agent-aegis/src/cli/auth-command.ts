...
const AUTH_COMMAND_USAGE: Record<AuthCommandKind, string> = {
	check: "aegis auth check --provider <provider> [--json] [--credentials] [--no-refresh]",
	api_key: "aegis auth print-api-key --provider <provider> [--model <model>]",
	bearer_token: "aegis auth print-bearer-token --provider <provider> [--model <model>] [--min-expiry <duration>]",
};
...
export function parseAuthCommand(args: string[]): AuthCommand | undefined {
	...
	if (!kind) {
		throw new AuthCommandError(
			`Unknown auth command "${args[1] ?? ""}". Use "aegis auth print-api-key", "aegis auth print-bearer-token", or "aegis auth check".`,
		);
	}
