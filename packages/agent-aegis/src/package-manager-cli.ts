...
Update aegis, installed packages, or model catalogs.

Options:
  --self                  Update aegis only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update aegis and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall aegis even if the current version is latest

Short forms:
  ${APP_NAME} update                Update aegis only
  ${APP_NAME} update --all          Update aegis and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update self           Update aegis only
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "aegis";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of aegis executable: ${entrypoint}`);
	}
}
